...
# Emprestar: cria um Emprestimo com usuário atual, marca livro como emprestado/não disponível
@app.route('/livro/emprestar/<int:livro_id>', methods=['POST'])
@login_required
def emprestar_livro(livro_id):
    if not current_user.is_admin:
        ...
        return redirect(url_for("index"))
    livro = Livro.query.get_or_404(livro_id)
    # verifica se já está emprestado ou vendido
    if livro.emprestado or livro.vendido or not livro.disponivel:
        flash("Livro não disponível para empréstimo.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
    emprestimo = Emprestimo(usuario_id=current_user.id, livro_id=livro.id)
...
# Cancelar empréstimo: remove empréstimo ativo (data_devolucao is None)
@app.route('/livro/cancelar_emprestimo/<int:livro_id>', methods=['POST'])
@login_required
def cancelar_emprestimo(livro_id):
    if not current_user.is_admin:
        flash("Acesso restrito!", "danger")
        return redirect(url_for("index"))
    emprestimo = Emprestimo.query.filter_by(livro_id=livro_id, data_devolucao=None).first()
    if not emprestimo:
        flash("Nenhum empréstimo ativo encontrado.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
...
# Devolver por livro (quando estiver em editar): marca data_devolucao
@app.route('/livro/devolver_por_livro/<int:livro_id>', methods=['POST'])
@login_required
def devolver_livro_por_livro(livro_id):
    if not current_user.is_admin:
        flash("Acesso restrito!", "danger")
        return redirect(url_for("index"))
    emprestimo = Emprestimo.query.filter_by(livro_id=livro_id, data_devolucao=None).first()
    if not emprestimo:
        flash("Nenhum empréstimo ativo para devolver.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
    emprestimo.data_devolucao = datetime.now()
    livro = emprestimo.livro
    livro.disponivel = True
...
# Vender: cria Venda, marca livro vendido e não disponível
@app.route('/livro/vender/<int:livro_id>', methods=['POST'])
@login_required
def vender_livro(livro_id):
    ...
    livro = Livro.query.get_or_404(livro_id)
    if livro.vendido:
        flash("Livro já está marcado como vendido.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
    # impede venda se emprestado
    if livro.emprestado:
        flash("Não é possível vender um livro que está emprestado.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
    venda = Venda(usuario_id=current_user.id, livro_id=livro.id, preco=livro.preco)
    livro.vendido = True
    livro.disponivel = False
    db.session.add(venda)
    db.session.commit()
    flash("Livro vendido.", "success")
    return redirect(url_for('editar_livro', livro_id=livro_id))

# Cancelar venda: apaga última venda do livro e reverte flags
@app.route('/livro/cancelar_venda/<int:livro_id>', methods=['POST'])
@login_required
def cancelar_venda(livro_id):
    if not current_user.is_admin:
        flash("Acesso restrito!", "danger")
        return redirect(url_for("index"))
    venda = Venda.query.filter_by(livro_id=livro_id).order_by(Venda.data_venda.desc()).first()
    livro = Livro.query.get_or_404(livro_id)
    if not venda:
        flash("Nenhuma venda encontrada para cancelar.", "warning")
        return redirect(url_for('editar_livro', livro_id=livro_id))
